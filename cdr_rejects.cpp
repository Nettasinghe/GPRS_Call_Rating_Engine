...
//  File		: cdr_rejects.cpp 
//  Description : cdr_rejects class implementation 
//  Date		: 02 Jan 2002 
...
bool cdr_rejects::is_reject_uplink_downlink(int file_id, int file_pos, char* uplink_downlink)
{
	detailed_cdr_id a;
	a.file_id=file_id;
	a.file_position=file_pos;
	a.uplink_downlink=uplink_downlink;

	return (reject_detailed_cdr_list.find(a) != reject_detailed_cdr_list.end());
}
...
{
	for(set<detailed_cdr_id>::iterator i=reject_detailed_cdr_list.begin(); i!=reject_detailed_cdr_list.end(); i++)
		cout<< (*i).file_id<<" "<<(*i).file_position<<" "<<(*i).uplink_downlink<< endl;
...
bool cdr_rejects::detailed_cdr_id::operator < (const detailed_cdr_id &s) const
{
	if     (file_id < s.file_id)
		return true;
	else if(file_id == s.file_id &&
			file_position < s.file_position)
		return true;
	else if(file_id == s.file_id && file_position == s.file_position && uplink_downlink < s.uplink_downlink)
		return true;
...
void cdr_rejects::insert(int file_id, int file_pos, char* uplink_downlink)
{
	detailed_cdr_id a;
	a.file_id=file_id;
	a.file_position=file_pos;
	...
	reject_detailed_cdr_list.insert(a);
}
