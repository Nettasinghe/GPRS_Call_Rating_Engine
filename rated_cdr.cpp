...
//  File		: rated_cdr.cpp 
//  Description : rated_cdr class implementation 
//  Date		: 02 Jan 2002 
...
bool rated_cdr::unpack_Uplink(gprs_cdr_decode *cdr, cdr_file_statistics *s)
{
	...
	*/
	...
	IMSI_DEF::iterator i= gsm_connections->gprs_imsi_map.find(servedIMSI);
	if (i == gsm_connections->gprs_imsi_map.end())
	{
		//imsi_found = false;
	...
	{
		//imsi_found =  true;
		connection_d.account_no = (*i).second.account_no;
		CLIENT_DEF::iterator iter = gsm_connections->gprs_client_map.find((*i).second.account_no);

		if(iter == gsm_connections->gprs_client_map.end())
		{
		}
		else
		{
			rate_group_id = (*iter).second.rate_group_id;
		}
	}
