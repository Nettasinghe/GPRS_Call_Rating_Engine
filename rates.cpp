...
//  File		: rates.cpp 
//  Description : rates class implementation 
//  Date		: 02 Jan 2002 
...
bool rates::match(string rate_group, string a_locale_id, rated_cdr &cdr, rating_observer &observer)
{
	...
	rk.call_type_id		= cdr.type_of_call; 
	//cout << "cdr.service_type_id : "<< cdr.service_type_id << endl;


	//  not in customer data base and local imsi, make into class method - TODO
	...
	/*
	for (RATE_DEF::iterator it=rate_data.begin(); it!=rate_data.end(); it++)
	{
		cout <<"Rate data first :" << (*it).first << endl;
		cout <<"Rate data second:" << (*it).second << endl;
	}
	*/
	...
	else
	{
		...
		//observer.units			= ((float)cdr.call_duration)/observer.unit_duration;
